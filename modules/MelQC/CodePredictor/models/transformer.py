...
class Attention(nn.Module):
    ...
    def forward(self, x, context=None, attn_mask=None, attn_bias=None):
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, x, effect_lengths, us_rate=1, attn_bias=None):
        ...
        for i in range(B):
            attn_mask[
                i, : effect_lengths[i] * us_rate, : effect_lengths[i] * us_rate
            ] = attn_mask_max[
                : effect_lengths[i] * us_rate, : effect_lengths[i] * us_rate
            ]
        ...
        attention = self.attention(
            xn, xn, attn_mask=attn_mask, attn_bias=attn_bias
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(
        self, x, c, effect_lengths, us_rate=1, attn_bias_self=None, attn_bias_cross=None
    ):
        ...
        attn_mask_cross = (
            attn_mask_cross.unsqueeze(1).repeat(1, us_rate, 1).contiguous().view(L, S)
        )
        ...
        for i in range(B):
            attn_mask2[i, : effect_lengths[i] * us_rate, : effect_lengths[i]] = (
                attn_mask_cross[: effect_lengths[i] * us_rate, : effect_lengths[i]]
            )
        ...
        attention = self.attention1(
            xn, xn, attn_mask=attn_mask1, attn_bias=attn_bias_self
        )
        x = self.dropout1(attention) + x  # residual connection

        xn = self.norm2(x)
        c = self.cnorm(c)
        attention = self.attention2(
            xn, c, attn_mask=attn_mask2, attn_bias=attn_bias_cross
        )
...
class CodePredictor(nn.Module):
    def __init__(
        ...
        self.layers_en = nn.ModuleList(
            [
                TransformerEncoderBlock(
                    embed_size, heads, dropout, forward_expansion, window_length
                )
        ...
        self.proj_out = nn.ModuleList(
            [nn.Linear(embed_size, n_labels) for _ in range(n_classifiers)]
        )
    ...
    def encode(self, vfeat, effect_lengths):
        if self.us_rate != 1:
            vfeat = F.interpolate(
                vfeat.permute(0, 2, 1), scale_factor=self.us_rate, mode="linear"
            )
        ...
        attn_bias_enc = self.rel_pos_bias(
            L, L, int(L / self.seq_length), int(L / self.seq_length)
        )
        ...
        for enc_layer in self.layers_en:
            enc_output = enc_layer(
                enc_output,
                effect_lengths,
                us_rate=int(L / self.seq_length),
                attn_bias=attn_bias_enc,
            )
