...
class MyDataModule(pl.LightningDataModule):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.train_dataset = VGGSoundQmelDataset(
            "train", args.root_path, num_quan=args.num_quan, num_win=args.num_win
        )
        self.test_dataset = VGGSoundQmelDataset(
            "test", args.root_path, num_quan=args.num_quan, num_win=args.num_win
        )
...
class VideoCodePredictor(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {
            "loss": res["loss"],
            "loss_pred": res["loss_pred"],
            "loss_mean": res["loss_mean"],
            "loss_std": res["loss_std"],
        }
    ...
    def get_pred_shared_step(self, batch, batch_idx):
        ...
        loss_pred = 0
        assert len(prediction) == code.shape[2], (
            "Predict and Target length: {}, {}".format(len(prediction), code.shape[2])
        )
        ...
        for i in range(len(prediction)):
            loss_pred += (
                torch.sum(
                    self.criterion(
                        prediction[i].contiguous().view(B * L, C),
                        code[:, :, i].view(B * L),
                    )
                    * effect_mask_flatten
                )
                / torch.sum(effect_mask_flatten)
        ...
        pred_std = pred_ms[:, :, 1]
        loss_mean = torch.sum(
            self.criterion_ms(pred_mean, mean).contiguous().view(B * L)
            * effect_mask_flatten
        ) / torch.sum(effect_mask_flatten)
        loss_std = torch.sum(
            self.criterion_ms(pred_std, std).contiguous().view(B * L)
            * effect_mask_flatten
        ) / torch.sum(effect_mask_flatten)
    ...
    def compute_Metric(self, res):
        ...
        metrics = {
            "Loss": res["loss"].detach().cpu().numpy(),
            "Acc": acc.detach().cpu().numpy(),
            "Nsample": torch.sum(effect_mask).detach().cpu().numpy(),
        }
    ...
    def get_valid_shared_end_step(self):
        loss, acc = (
            round(
                np.sum(
                    np.array(self.Metrics["Loss"]) * np.array(self.Metrics["Nsample"])
                )
                / np.sum(self.Metrics["Nsample"]),
                4,
            ),
            round(
                np.sum(
                    np.array(self.Metrics["Acc"]) * np.array(self.Metrics["Nsample"])
                )
                / np.sum(self.Metrics["Nsample"])
                * 100,
                2,
        ...
        loss_pred = round(
            np.sum(
                np.array(self.Metrics["Loss_pred"]) * np.array(self.Metrics["Nsample"])
            )
            / np.sum(self.Metrics["Nsample"]),
            4,
        )
        loss_mean = round(
            np.sum(
                np.array(self.Metrics["Loss_mean"]) * np.array(self.Metrics["Nsample"])
            )
            / np.sum(self.Metrics["Nsample"]),
            4,
        )
        loss_std = round(
            np.sum(
                np.array(self.Metrics["Loss_std"]) * np.array(self.Metrics["Nsample"])
            )
            / np.sum(self.Metrics["Nsample"]),
            4,
        )
        ...
        self.log(
            f"Best@Loss_pred",
            self.BestMetrics["Loss_pred"],
            prog_bar=True,
            sync_dist=True,
        )
        self.log(
            f"Best@Loss_mean",
            self.BestMetrics["Loss_mean"],
            prog_bar=True,
            sync_dist=True,
        )
        self.log(
            f"Best@Loss_std",
            self.BestMetrics["Loss_std"],
            prog_bar=True,
            sync_dist=True,
        )
        self.log(
            f"Best@Epoch", self.BestMetrics["Epoch"], prog_bar=True, sync_dist=True
        )
        self.Metrics = {
            "Loss": [],
            "Acc": [],
...
def args_parse():
    ...
    args.add_argument(
        "--num_win", type=int, default=8, help="number of frequency windows"
    )
    args.add_argument(
        "--num_warmup_steps", type=float, default=0.1, help="ratio of warmup steps"
    )
    args.add_argument(
        "--lr", type=float, default=1e-4, help="learning rate for AdamW optimizer"
    )
...
if __name__ == "__main__":
    ...
    num_training_steps = args.epochs * int(
        PLData.train_dataset.__len__() / (args.batch_size * args.num_device)
    )
