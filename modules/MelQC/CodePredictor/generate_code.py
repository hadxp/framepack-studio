...
class VideoCodeGenerator:
    def __init__(self, ckpt_path: str, num_win: int, num_quan: int):
        self.num_win = num_win
        self.num_quan = num_quan
        n_classifiers = num_win // 4
        n_labels = np.power(num_quan, 4)
        tseq_size = num_win
        us_rate = 4
    ...
    def pred2emb(self, prediction, mean=None, std=None):
        emb = code2emb_qmel(prediction, num_quan=self.num_quan, num_win=self.num_win)
        return emb
    ...
    def generate(self, vfeat, effect_lengths=None, mean=None, std=None):
        ...
        with torch.no_grad():
            ...
            prediction_cat = (
                torch.cat(prediction, dim=0)
                .contiguous()
                .view(self.n_classifiers, B, L, C)
                .permute(1, 2, 0, 3)
            )
...
if __name__ == "__main__":
    ...
    # Load test dataset
    test_dataset = VGGSoundQmelDataset_test(
        root_path + "/video_clip", num_quan=args.num_quan, num_win=args.num_win
    )
    ...
    for batch in test_dataloader:
        ...
        for i in range(len(video_indices)):
            ...
            np.save(
                os.path.join(save_path_mean, "{}.npy".format(video_index)), mean_pred[i]
            )
            np.save(
                os.path.join(save_path_std, "{}.npy".format(video_index)), std_pred[i]
            )
