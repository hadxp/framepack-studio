...
class VGGSoundQmelDataset(data.Dataset):
    def __init__(self, mode, root_path, num_quan=3, num_win=8):
        ...
        self.mean_root = os.path.join(
            self.root_path, "Qmel_{}_{}".format(num_quan, num_win), "Mean"
        )
        self.std_root = os.path.join(
            self.root_path, "Qmel_{}_{}".format(num_quan, num_win), "Std"
        )
        self.emb_root = os.path.join(
            self.root_path, "Qmel_{}_{}".format(num_quan, num_win), "Emb"
        )
        self.code_root = os.path.join(
            self.root_path, "Qmel_{}_{}".format(num_quan, num_win), "Code"
        )
    ...
    def __getitem__(self, index):
        ...
        mean = F.pad(
            mean[:effect_length_audio],
            (0, 250 * (effect_length_audio // effect_length) - effect_length_audio),
            value=0,
        )
        std = F.pad(
            std[:effect_length_audio],
            (0, 250 * (effect_length_audio // effect_length) - effect_length_audio),
            value=0,
        )
        emb = F.pad(
            emb[:effect_length_audio],
            (
                0,
                0,
                0,
                250 * (effect_length_audio // effect_length) - effect_length_audio,
            ),
            value=0,
        )
        code = F.pad(
            code[:effect_length_audio],
            (
                0,
                0,
                0,
