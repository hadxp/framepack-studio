...
def compress_to_file(
    model: EncodecModel, wav: torch.Tensor, fo: tp.IO[bytes], use_lm: bool = True
):
...
def decompress_from_file(fo: tp.IO[bytes], device="cpu") -> tp.Tuple[torch.Tensor, int]:
    ...
    for offset in range(0, audio_length, segment_stride):
        ...
        frame_length = int(
            math.ceil(this_segment_length * model.frame_rate / model.sample_rate)
        )
        if model.normalize:
...
def test():
    ...
    for name in MODELS.keys():
        ...
        for use_lm in [False, True]:
            ...
            print(
                f"kbps: {kbps:.1f}, time comp: {t_comp:.1f} sec. "
