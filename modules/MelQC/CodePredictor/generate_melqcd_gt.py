...
def normalize_spectrogram(spectrogram, max_value=200, min_value=1e-5, power=1.0):
    # H, W
    ...
    if spectrogram.shape[1] != 1024:
        spectrogram = np.pad(
            spectrogram,
            ((0, 0), (0, 1024 - spectrogram.shape[1])),
            "constant",
...
def quantize_spectrogram(spectrogram, clamp_value=3):
    # H, W
    Mean, Std = (
        np.mean(spectrogram, axis=0, keepdims=True),
...
def norm_and_quantize(mel, clamp_val=3):
    ...
    spec_class, spec_m, spec_s, Min, Max = quantize_spectrogram(normed_mel, clamp_val)
...
if __name__ == "__main__":
    ...
    for save_name in tqdm(file_names):
        ...
        try:
            ...
            assert H == 256
            assert W <= 1024
            mel = rearrange(mel, "(a b) w -> a b w", a=NumWin, b=H // NumWin).mean(
                axis=1
            )
            spec_class, spec_m, spec_s, Min, Max = norm_and_quantize(mel, NumSide)

            np.save(
                os.path.join(save_root, "Mean", "{}.npy".format(save_name)), spec_m[0]
            )
            np.save(
                os.path.join(save_root, "Std", "{}.npy".format(save_name)), spec_s[0]
            )
            # W * NumWin
            np.save(
                os.path.join(save_root, "Emb", "{}.npy".format(save_name)),
                spec_class.transpose(1, 0),
            )
            codes = rearrange(
                spec_class.transpose(1, 0), "w (a b) -> w a b", a=NumWin // 4, b=4
            )
            mult = np.power(NumQuan, np.arange(4))
            codes = np.sum((codes + NumSide) * mult[None, None, :], axis=2)
            np.save(
                os.path.join(save_root, "Code", "{}.npy".format(save_name)),
                codes.astype(int),
            )
