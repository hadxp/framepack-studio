...
def cfg_sanity_check_and_patch(cfg):
    if cfg.action == "train_avclip":
        assert cfg.model.params.afeat_extractor.params.get(
            "add_global_repr"
        ) == cfg.model.params.vfeat_extractor.params.get("add_global_repr"), (
    ...
    assert not (cfg.training.resume and cfg.training.finetune), (
        "it is either funetuning or resuming"
    )
    assert not (cfg.training.resume and cfg.training.run_test_only), (
        "it is either resuming or testing-only"
    )
    assert not (cfg.training.finetune and cfg.training.run_test_only), (
    ...
    if offset_type is not None:
        if "grid" in offset_type and "loss_fn" in cfg.training:
            assert "mse" not in cfg.training.loss_fn[1], (
                f"to class but loss: {cfg.training.loss_fn[1]}"
            )
        elif "uniform" in offset_type:
            assert "cross_entropy" not in cfg.training.loss_fn[1], (
    ...
    if afeat_extractor.endswith("ResNet18AudioFeatures") and vfeat_extractor.endswith(
        ...
        assert cfg.logging.vis_segment_sim is False, (
