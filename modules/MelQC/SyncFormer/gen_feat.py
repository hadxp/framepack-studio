...
def process(args):
    ...
    for i, video_path in tqdm(enumerate(video_paths), desc=f"{len(video_paths)}"):
        try:
            ...
            if os.path.exists(
                os.path.join(tar_root + "/syncformer/", f"{base_name}.npy")
            ):
                continue
            if os.path.exists(
                os.path.join(tar_root + "/CLIP_LargeEmbed/", f"{base_name}.npy")
            ):
                continue
            ...
            np.save(
                os.path.join(tar_root + "/syncformer/", f"{base_name}.npy"), vfeat_sync
            )
