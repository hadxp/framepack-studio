...
def softmax_kernel_transformation(
    ...
    if is_query:
        data_dash = ratio * (
            torch.exp(data_dash - diag_data - data_dash.amax(last_dims_t, True))
        ...
        data_dash = ratio * (
            torch.exp(
                data_dash
                - diag_data
                - data_dash.amax(last_dims_t + attention_dims_t, True)
            )
            + numerical_stabilizer
        )
...
def favor_attention(query, key, value, projection_matrix=None):
    ...
    attention_normalizer = attention_normalizer.unsqueeze(
...
def get_attention(
    ...
    if projection_matrix_type is None:
        ...
        projection_matrix = create_projection_matrix(nb_random_features, dim, seed=seed)
...
def test_softmax_noncausal_attention_block_output():
    ...
    exact_attention_block_output = torch.einsum(
        "bxyh,byhd->bxyh", attention_scores, value
    )
    max_error = 0.5
