    ...
    def forward(self, x, seq_len=196, num_frames=8, approx="none", num_landmarks=128):
        ...
        if approx == "nystrom":
            ...
            q_prime = performer_helper.softmax_kernel_transformation(
                q_, is_query=True, projection_matrix=projection_matrix
            )
            k_prime = performer_helper.softmax_kernel_transformation(
                k_, is_query=False, projection_matrix=projection_matrix
            )
            q_prime, k_prime = map(
            ...
            normaliser = torch.einsum(
                "b h p r, b h f r -> b h p f", q_prime, normaliser
            )
