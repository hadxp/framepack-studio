...
from model.modules.feat_extractors.visual.motionformer import (
    AveragePooling,
...
class S3D(nn.Module):
    def __init__(self, num_class, extract_features):
        ...
        self.fc = nn.Sequential(
            nn.Conv3d(self.embed_dim, num_class, kernel_size=1, stride=1, bias=True),
        )
...
class S3DVisualFeatures(S3D):
    ...
    def forward(self, x, for_loop: bool = False, cont_mask: torch.Tensor = None):
        ...
        global_x = None
        if (
            self.extract_features and self.add_global_repr
        ):  # lazy exec:, add_global_repr might not be there
            assert len(x.shape) == 3, (
                f"Local representation should be (B, S, D) {x.shape}"
            )
