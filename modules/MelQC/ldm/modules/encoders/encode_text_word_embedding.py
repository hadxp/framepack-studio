...
def encode_text_word_embedding(
    ...
    """
    existing_indexes = (input_ids == 259).nonzero(as_tuple=True)[
        0
    ...
    if len(existing_indexes) > 0:  # if there are '$' in the text
        _, counts = torch.unique(
            (input_ids == 259).nonzero(as_tuple=True)[0], return_counts=True
        )
        cum_sum = torch.cat(
            (
                torch.zeros(1, device=input_ids.device).int(),
                torch.cumsum(counts, dim=0)[:-1],
            )
        )
        first_vstar_indexes = (input_ids == 259).nonzero()[cum_sum][
            :, 1
        ]  # get the index of the first '$' in each sentence
        rep_idx = torch.cat(
            [(first_vstar_indexes + n).unsqueeze(0) for n in range(num_vstar)]
        )
    ...
    if len(existing_indexes) > 0:
        ...
        input_embeds[
            torch.arange(input_embeds.shape[0])
            .repeat_interleave(num_vstar)
            .reshape(input_embeds.shape[0], num_vstar)[existing_indexes.cpu()],
            rep_idx.T,
        ] = word_embeddings.to(input_embeds.dtype)[
            existing_indexes
        ]  # replace the '$' with the PTEs
