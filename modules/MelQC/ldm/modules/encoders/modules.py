...
from ldm.modules.encoders.inversion_adapter import InversionAdapter
from ldm.modules.encoders.encode_text_word_embedding import encode_text_word_embedding
...
class LearnableTextualInversion(AbstractEncoder):
    """Uses the CLIP transformer encoder for text (from huggingface)"""
    ...
    def __init__(
        ...
        self.inversion_adapter = InversionAdapter(
            input_dim=vision_encoder.config.hidden_size,
            hidden_dim=vision_encoder.config.hidden_size * 4,
            output_dim=self.transformer.config.hidden_size * self.num_vstar,
    ...
    def forward(self, text, img):
        text = [f"{txt}, {' $ ' * self.num_vstar}" for txt in text]
        batch_encoding = self.tokenizer(
            text,
            truncation=True,
            max_length=self.max_length,
        ...
        word_embeddings = self.inversion_adapter(img)  # global img
        bsz = len(text)

        word_embeddings = word_embeddings.reshape((bsz, self.num_vstar, -1))

        tokens = batch_encoding["input_ids"].to(self.device)
        encoder_hidden_states = encode_text_word_embedding(
            self.transformer, tokens, word_embeddings, self.num_vstar
        ).last_hidden_state
