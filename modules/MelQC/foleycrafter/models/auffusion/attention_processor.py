...
class IPAdapterAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        if scale != 1.0:
            logger.warning(
                "`scale` of IPAttnProcessor should be set with `set_ip_adapter_scale`."
            )
        residual = hidden_states
        ...
        for current_ip_hidden_states, scale, to_k_ip, to_v_ip, mask in zip(
            ...
            if not skip:
                ...
                for time_condition in time_conditions:
                    ...
                    time_condition_mask = (
                        time_condition_mask.flatten(-2).unsqueeze(-1).repeat(1, 1, 4)
                    )
                    if time_condition_masks is None:
                        time_condition_masks = time_condition_mask
                    else:
                        time_condition_masks = torch.cat(
                            [time_condition_masks, time_condition_mask], dim=-1
                        )

                current_ip_hidden_states = rearrange(
                    current_ip_hidden_states, "L B N C -> B (L N) C"
                )
                ...
                time_condition_masks = time_condition_masks.unsqueeze(1).repeat(
                    1, attn.heads, 1, 1
                )
                ...
                current_ip_hidden_states = F.scaled_dot_product_attention(
                    ...
                    ip_key,
                    ip_value,
                    attn_mask=time_condition_masks,
                    dropout_p=0.0,
                    is_causal=False,
                )
