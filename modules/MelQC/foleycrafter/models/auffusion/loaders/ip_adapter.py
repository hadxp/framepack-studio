...
class IPAdapterMixin:
        ...
        **kwargs,
        ...
        for pretrained_model_name_or_path_or_dict, weight_name, subfolder in zip(
            ...
            if (
                ...
                and getattr(self, "feature_extractor", None) is None
            ):
    ...
    def unload_ip_adapter(self):
        ...
        if (
            hasattr(self, "image_encoder")
            and getattr(self, "image_encoder", None) is not None
        ):
...
class VPAdapterMixin:
    ...
    def set_ip_adapter_scale(self, scale):
        ...
        for attn_processor in unet.attn_processors.values():
            if isinstance(
                attn_processor,
                (IPAdapterAttnProcessor, VPTemporalAdapterAttnProcessor2_0),
            ):
                if not isinstance(scale, list):
                    scale = [scale] * len(attn_processor.scale)
                if len(attn_processor.scale) != len(scale):
