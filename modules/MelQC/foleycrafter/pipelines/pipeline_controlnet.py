...
class StableDiffusionControlNetPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        sound_embeds, negative_prompt_embeds = self.encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
        ...
        silence_embeds, negative_prompt_embeds = self.encode_prompt(
            "there is no sound",
            device,
            num_images_per_prompt,
            self.do_classifier_free_guidance,
        ...
        negative_prompt_embeds = negative_prompt_embeds.unsqueeze(1).repeat(
            1, im_width, 1, 1
        )
        sound_embeds = sound_embeds.unsqueeze(1).repeat(1, im_width, 1, 1)
        silence_embeds = silence_embeds.unsqueeze(1).repeat(1, im_width, 1, 1)
        time_condition = image[:, 0, 0].unsqueeze(2).unsqueeze(3)
