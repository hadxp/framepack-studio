...
def random_audio_video_clip(
    ...
    video_start_frame, video_end_frame = (
        video_length * start_idx,
        video_length * end_idx,
    )
    audio_start_frame, audio_end_frame = (
        audio_length * start_idx,
        audio_length * end_idx,
    )
    ...
    audio_idx = np.linspace(
        audio_start_frame, audio_end_frame, sample_rate * duration, dtype=int
    )
...
def get_full_indices(
    reader: Union[decord.VideoReader, decord.AudioReader],
) -> np.ndarray:
...
def get_frames_in_video(
    video_path: str, onset_list, frame_nums=1024, audio_length_in_s=10
):
    ...
    for start, end in onset_list:
        if int(start / frame_nums * duration) >= audio_length_in_s:
            continue
        video_start = int(
            start / audio_length_in_s * duration / frame_nums * video_frame
        )
        if video_start >= video_frame:
            continue
        video_end = int(end / audio_length_in_s * duration / frame_nums * video_frame)
        video_onset_list.append(
            [
                int(start / audio_length_in_s * duration),
                int(end / audio_length_in_s * duration),
            ]
        )
        frames_list.extend(range(video_start, video_end))
    frames = video.get_batch(frames_list).asnumpy()
...
def composite_audio_video(
    ...
    save_multimodal(
        video, audio, path, audio_fps=audio_sample_rate, video_fps=video_fps
    )
    return
...
class SpectrogramConverter:
    ...
    def spectrogram_from_audio(
        ...
        """
        assert int(audio.frame_rate) == self.p.sample_rate, (
...
def convert_ldm_vae_checkpoint(
    checkpoint, config, only_decoder=False, only_encoder=False
):
    ...
    if only_decoder:
        new_checkpoint = {
            ...
            if k.startswith("decoder") or k.startswith("post_quant")
        }
    elif only_encoder:
        new_checkpoint = {
            k: v
...
def convert_lora_model_level(
    ...
    for key in state_dict:
        ...
        if "conv_in" in pair_keys[0]:
            ...
            shape[1] = 4
            curr_layer.weight.data[:, :4, ...] += alpha * (
...
def read_frames_with_moviepy_keepspeed(video_path, max_frame_nums=None):
    clip = VideoFileClip(video_path)
    duration = clip.duration
    frames = []
    for frame in clip.iter_frames():
    ...
    if max_frame_nums is not None:
        frames_idx = np.linspace(
            0, len(frames) - 1, int(max_frame_nums * duration / 10), dtype=int
        )
    return np.array(frames)[frames_idx, ...], duration
