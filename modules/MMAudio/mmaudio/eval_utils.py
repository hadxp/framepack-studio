...
def generate(
    ...
    *,
    ...
    # Workaround for PyTorch "Inference tensors cannot be saved for backward":
    # clone features returned from encoders so they are normal tensors, not "inference" tensors.
    if clip_video is not None and clip_features is not None:
        clip_features = clip_features.clone()
    if sync_video is not None and sync_features is not None:
        sync_features = sync_features.clone()
    if text is not None and text_features is not None:
        text_features = text_features.clone()
    if negative_text is not None and negative_text_features is not None:
        negative_text_features = negative_text_features.clone()
